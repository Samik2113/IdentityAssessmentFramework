namespace IamMaturityStudio.Application.Contracts;

public sealed record RequesterContext(Guid UserId, string Name, string Email, IReadOnlySet<string> Roles, IReadOnlyList<Guid> OrgMemberships);
...
public sealed record QuestionnaireTreeResponse(
    Guid QuestionnaireId,
    string Name,
    string Version,
    IReadOnlyList<QuestionnaireDomainNode> Domains);

public sealed record QuestionnaireDomainNode(
    Guid Id,
    string Code,
    string Name,
    IReadOnlyList<QuestionnaireCategoryNode> Categories);

public sealed record QuestionnaireCategoryNode(
    Guid Id,
    string Code,
    string Name,
    decimal Weight,
    string BusinessRisk,
    IReadOnlyList<QuestionnaireQuestionNode> Questions);
...
public sealed record AssessmentDetailForConsultantResponse(
    AssessmentSummaryResponse Assessment,
    decimal? OverallPercent,
    decimal? OverallMaturity0To5);

public sealed record AssessmentDetailForRespondentResponse(AssessmentSummaryResponse Assessment);
...
public sealed record ResponseListForConsultant(
    Guid QuestionId,
    string Level,
    string? Comment,
    int? Confidence,
...
public sealed record ResponseListForRespondent(
    Guid QuestionId,
    string Level,
    string? Comment,
    int? Confidence,
...
public sealed record DashboardResponse(
    DashboardKpi Kpis,
    IReadOnlyList<DomainScoreDto> Domains,
    IReadOnlyList<DashboardCategoryScore> Categories,
    IReadOnlyList<DashboardRadarSeries> RadarSeries,
...
public sealed record AiGuidanceResponse(
    string Explanation,
